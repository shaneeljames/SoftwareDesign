...
\subsubsection{Communication Diagrams}
The state diagrams are listed below, they depict the states in which various attributes in the the database change.\\

\textbf{State Diagram: "Status" in the Confrimed Session Table}\\\\
\text The state is changed both when the tutor has accepted the tutoring session.
\includegraphics[width=140mm]{./state_diagram/statediagram_status.png}

\text{State Diagram: "Paid" in the Tutor Student Table}\\\\
